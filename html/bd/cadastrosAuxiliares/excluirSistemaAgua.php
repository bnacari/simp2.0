...
try {
    ...
    $identificador = "ID: $id";
    try {
        $stmtBusca = $pdoSIMP->prepare("SELECT * FROM SIMP.dbo.SISTEMA_AGUA WHERE CD_CHAVE = :id");
    ...
    $sql = "DELETE FROM SIMP.dbo.SISTEMA_AGUA WHERE CD_CHAVE = :id";
    $stmt = $pdoSIMP->prepare($sql);
    $stmt->execute([':id' => $id]);
    
    if (function_exists('registrarLogDelete')) { try { registrarLogDelete('Cadastros Auxiliares', 'Sistema de Água', $id, $identificador, $dadosExcluidos); } catch (Exception $e) {} }
